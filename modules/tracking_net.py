...
class TrackingNet(nn.Module):
    ...
    def associate(self, objs, dets):
        # link_mat, new_score, end_score = self.w_link(objs, dets)   # link_mat:(3, 1, N, M)
        link_mat = self.w_link(objs, dets)   # link_mat:(3, 1, N, M)
...
#             det_scores[:, -1] -= mask.float()
    ...
    def forward(self, dets, det_info, dets_split):
        ...
        for i in range(len(dets_split) - 1):
            ...
            link_mat = self.associate(feats[:, :, start:prev_end], feats[:, :, prev_end:end])
